...
func (k Source) MountKVStores(app interface{}, fieldName string) error {
	keys, ok := getFieldUsingReflection(app, fieldName).(map[string]*sdktypes.KVStoreKey)
	if !ok {
		return fmt.Errorf("error while getting keys")
	}

	for _, key := range keys {
		k.Cms.MountStoreWithDB(key, sdktypes.StoreTypeIAVL, nil)
	}
...
func (k Source) MountTransientStores(app interface{}, fieldName string) error {
	tkeys, ok := getFieldUsingReflection(app, fieldName).(map[string]sdktypes.StoreKey)
	if !ok {
		return fmt.Errorf("error while getting transient keys")
	}

	for _, key := range tkeys {
		k.Cms.MountStoreWithDB(key, sdktypes.StoreTypeTransient, nil)
	}
...
func (k Source) MountMemoryStores(app interface{}, fieldName string) error {
	memKeys, ok := getFieldUsingReflection(app, fieldName).(map[string]*sdktypes.MemoryStoreKey)
	if !ok {
		return fmt.Errorf("error while getting memory keys")
	}

	for _, key := range memKeys {
		k.Cms.MountStoreWithDB(key, sdktypes.StoreTypeMemory, nil)
	}
